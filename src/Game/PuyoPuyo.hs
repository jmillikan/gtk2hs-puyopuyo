...
module Game.PuyoPuyo (newGameState, gameStep, getGrid, getScore, gridWidth, gridHeight, GameState, PuyoColor(..), GameInput(..)) where
...
data GameInput = Tick | GIDown | GILeft | GIRight | RotLeft | RotRight
               deriving (Eq, Show)
...
gameStep g@(ControlBlock queue grid bl score) input = 
    case input of
      Tick       -> tryDrop
      GILeft  -> tryShift (_2 %~ (+%) (0,-1)) 
      GIRight -> tryShift (_2 %~ (+%) (0,1)) 
      GIDown  -> tryDrop
